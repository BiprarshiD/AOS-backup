...
def send(msg, pipe):
    ...
    try:
        ...
        if(rank==0):
            comm.send(payload,dest=1)         #server to player0
        elif (rank==1):
            comm.send(payload,dest=0)         #player0 to server
...
def recv(pipe):
    ...
    try:
        ...
        if(rank==0):
            output = comm.recv(source=1)         #player0 to server
        elif (rank==1):
            output = comm.recv(source=0)         #server to player0
