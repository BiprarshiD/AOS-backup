...
def send(msg, pipe):
    ...
    #check who the moderator is sending the msg to
    receiver = pipe.split('to')[1]
    if(receiver=='0'):
        client_mq = "/message_queue_player0"
    elif(receiver=='1'):
        client_mq = "/message_queue_player1"
    elif(receiver=='2'):
        client_mq = "/message_queue_player2"
    elif(receiver=='3'):
        client_mq = "/message_queue_player3"

    mqplayer = posix_ipc.MessageQueue(client_mq)
    sender = pipe.split('to')[0]
    payload=':' + sender + ':' + msg
    mqplayer.send(payload)
...
def recv(pipe):
    ...
    sender = pipe.split('to')[0]
    if(sender=='0'):
        output, _ = mq0.receive()      #server receives message
    elif(sender=='1'):
        output, _ = mq1.receive()      #server receives message
    elif(sender=='2'):
        output, _ = mq2.receive()      #server receives message
    elif(sender=='3'):
        output, _ = mq3.receive()      #server receives message
    ...
    try:
        ...
        for i in range(len(output)):
            if len(output[i]) > 0:
                out = pipe.split('to')[0]
                if ((output[i]=='s') or (output[i]==out) or (imposterMode==1)):
                    return output
    except (Exception, p):
